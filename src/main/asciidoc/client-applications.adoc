...
In languages where <<developer-manual#term-thread-safety, thread safety>> is an issue, the driver can be considered thread-safe.
...
This kind of driver uses the <<developer-manual#term-bolt-routing-protocol, Bolt Routing Protocol>> and works in tandem with the cluster to route transactions to available cluster members.
