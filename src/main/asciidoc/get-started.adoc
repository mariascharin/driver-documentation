...
See also <<developer-manual#http-api>>.

[[driver-get-started-installation]]
== Driver versions and installation

The 1.x series drivers have been built for Neo4j 3.x.
The driver major version number correlates with the  <<developer-manual#term-bolt, Bolt protocol>> version, the minor version number describes the driver feature set and the patch number defines the patch level of the driver in the regular sense.
