...
In languages where <<developer-manual#term-thread-safety, thread safety>> is an issue, a session should _not_ be considered thread-safe.
...
Parameter values should adhere to <<developer-manual#cypher-values>>.
...
However, Auto-commit transactions are the only way to execute `<<developer-manual#query-using-periodic-commit-hint, USING PERIODIC COMMIT>>` Cypher statements.
...
Each such exception is associated with a <<developer-manual#status-codes, status code>> that describes the nature of the error and a message that provides more detail.
...
Causal chaining is carried out by passing <<developer-manual#term-bookmark, bookmarks>> between transactions in a session.
